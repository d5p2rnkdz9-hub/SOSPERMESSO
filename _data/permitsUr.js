/**
 * 11ty data file for UR permit pages
 * Fetches permit data from UR Notion database, parses Q&A content into HTML
 * Slugs are resolved via IT Page ID → IT slug mapping (UR slugs = IT slugs)
 */
...
// UR Notion database ID (hardcoded like IT — no env var needed)
const UR_DATABASE_ID = '42ef74d0-62cf-4c3e-b5d8-eafd4b2155b8';
 ...
 * UR version: no linkToDizionario, just escapeHtml for all text
 */
...
/**
 * Get emoji based on permit type keywords (UR + IT)
 */
function getEmojiForPermit(tipo) {
  ...
  // UR keywords
  if (t.includes('تعلیم') || t.includes('studio')) return '📚';
  if (t.includes('عمل') || t.includes('lavoro subordinato') || t.includes('lavoro autonomo')) return '💼';
  if (t.includes('انتظار') || t.includes('بحث عن عمل') || t.includes('attesa occupazione') || t.includes('attesa lavoro')) return '🔍';
  if (t.includes('حفاظت') || t.includes('پناہ') || t.includes('مہاجر') || t.includes('protezione') || t.includes('rifugiato') || t.includes('asilo')) return '🛡️';
  if (t.includes('خاندان') || t.includes('خاندانی اتحاد') || t.includes('famiglia') || t.includes('familiare') || t.includes('ricongiungimento')) return '👨‍👩‍👧‍👦';
  if (t.includes('صحت') || t.includes('علاج') || t.includes('حمل') || t.includes('medic') || t.includes('salute') || t.includes('gravidanza') || t.includes('cure')) return '🏥';
  if (t.includes('طویل مدتی اقامت') || t.includes('soggiornante') || t.includes('lungo')) return '🏠';
  if (t.includes('مذہب')) return '⛪';
  if (t.includes('کھیل')) return '⚽';
  if (t.includes('فن')) return '🎭';
  if (t.includes('سائنسی تحقیق')) return '🔬';
  if (t.includes('نابالغ') || t.includes('minore')) return '👶';
  if (t.includes('آفت') || t.includes('calamit')) return '🌊';

  return '📋';
}

/**
 * Fetch and transform UR permit data from Notion
 */
module.exports = async function() {
  if (!process.env.NOTION_FETCH) {
    try {
      const cached = JSON.parse(require('fs').readFileSync(
        require('path').join(__dirname, '..', '_cache', 'permits-ur.json'), 'utf-8'
      ));
      console.log(`[permitsUr.js] Using cached data (${cached.length} permits)`);
      return notionLinks.fixLinksInPermits(cached.map(p => ({ ...p, rinnovoDocuments: (p.rinnovoDocuments || []).filter(d => d.toLowerCase() !== 'n/a') })), 'ur/');
    } catch { /* no cache, fall through to Notion fetch */ }
  }

  if (!process.env.NOTION_API_KEY) {
    console.warn('[permitsUr.js] NOTION_API_KEY not set - returning empty array');
    return [];
  }

  if (!UR_DATABASE_ID) {
    console.warn('[permitsUr.js] UR_DATABASE_ID not set - returning empty array');
    return [];
  }

  try {
    console.log('[permitsUr.js] Fetching UR permit data from Notion...');
    const notion = new Client({ auth: process.env.NOTION_API_KEY });

    // Build IT page ID → slug map (for resolving UR slugs)
    console.log('[permitsUr.js] Building IT slug map...');
    const itSlugMap = await buildItSlugMap(notion);
    console.log(`[permitsUr.js] IT slug map has ${Object.keys(itSlugMap).length} entries`);

    // Fetch UR pages
    const urPages = await fetchDatabasePages(notion, UR_DATABASE_ID);
    console.log(`[permitsUr.js] Found ${urPages.length} UR permit pages`);

    if (urPages.length === 0) {
      console.warn('[permitsUr.js] No UR permit data found');
      return [];
    }
    ...
    for (const page of urPages) {
      const tipo = page.properties["Name"]?.title?.[0]?.plain_text || null;
      if (!tipo) {
        console.warn(`[permitsUr.js] Skipping page ${page.id} - no Name`);
        continue;
      }
      ...
      if (!slug) {
        console.warn(`[permitsUr.js] Skipping "${tipo}" - no IT slug found (IT Page ID: ${itPageId || 'missing'})`);
        continue;
      }

      count++;
      if (count % 10 === 0) {
        console.log(`[permitsUr.js] Processed ${count}/${urPages.length} permits...`);
      }

      // Extract doc fields from UR page properties
      ...
      try {
        ...
        console.error(`[permitsUr.js] Error processing ${tipo}: ${err.message}`);
        processedPermits.push({
          id: page.id,
          slug,
          tipo,
    ...
    console.log(`[permitsUr.js] Cache: ${cacheHits} hits, ${cacheMisses} misses`);
    console.log(`[permitsUr.js] Returning ${processedPermits.length} UR permits`);
    return processedPermits;

  } catch (error) {
    console.error(`[permitsUr.js] Notion fetch failed: ${error.message}`);
    return [];
  }
};
