/**
 * 11ty data file for FA permit pages
 * Fetches permit data from FA Notion database, parses Q&A content into HTML
 * Slugs are resolved via IT Page ID → IT slug mapping (FA slugs = IT slugs)
 */
...
// FA Notion database ID (hardcoded like IT — no env var needed)
const FA_DATABASE_ID = 'e350cb8e-515c-45f6-8c72-13175ab574d1';
 ...
 * FA version: no linkToDizionario, just escapeHtml for all text
 */
...
/**
 * Get emoji based on permit type keywords (FA + IT)
 */
function getEmojiForPermit(tipo) {
  ...
  // FA keywords
  if (t.includes('تحصیل') || t.includes('studio')) return '📚';
  if (t.includes('کار') || t.includes('شغل') || t.includes('lavoro subordinato') || t.includes('lavoro autonomo')) return '💼';
  if (t.includes('انتظار') || t.includes('جستجوی کار') || t.includes('attesa occupazione') || t.includes('attesa lavoro')) return '🔍';
  if (t.includes('حمایت') || t.includes('پناهندگی') || t.includes('پناهنده') || t.includes('protezione') || t.includes('rifugiato') || t.includes('asilo')) return '🛡️';
  if (t.includes('خانواده') || t.includes('پیوند خانوادگی') || t.includes('famiglia') || t.includes('familiare') || t.includes('ricongiungimento')) return '👨‍👩‍👧‍👦';
  if (t.includes('سلامت') || t.includes('درمان') || t.includes('بارداری') || t.includes('medic') || t.includes('salute') || t.includes('gravidanza') || t.includes('cure')) return '🏥';
  if (t.includes('اقامت بلندمدت') || t.includes('soggiornante') || t.includes('lungo')) return '🏠';
  if (t.includes('مذهب')) return '⛪';
  if (t.includes('ورزش')) return '⚽';
  if (t.includes('هنر')) return '🎭';
  if (t.includes('تحقیقات علمی')) return '🔬';
  if (t.includes('صغیر') || t.includes('minore')) return '👶';
  if (t.includes('بلایای طبیعی') || t.includes('calamit')) return '🌊';

  return '📋';
}

/**
 * Fetch and transform FA permit data from Notion
 */
module.exports = async function() {
  if (!process.env.NOTION_FETCH) {
    try {
      const cached = JSON.parse(require('fs').readFileSync(
        require('path').join(__dirname, '..', '_cache', 'permits-fa.json'), 'utf-8'
      ));
      console.log(`[permitsFa.js] Using cached data (${cached.length} permits)`);
      return notionLinks.fixLinksInPermits(cached.map(p => ({ ...p, rinnovoDocuments: (p.rinnovoDocuments || []).filter(d => d.toLowerCase() !== 'n/a') })), 'fa/');
    } catch { /* no cache, fall through to Notion fetch */ }
  }

  if (!process.env.NOTION_API_KEY) {
    console.warn('[permitsFa.js] NOTION_API_KEY not set - returning empty array');
    return [];
  }

  if (!FA_DATABASE_ID) {
    console.warn('[permitsFa.js] FA_DATABASE_ID not set - returning empty array');
    return [];
  }

  try {
    console.log('[permitsFa.js] Fetching FA permit data from Notion...');
    const notion = new Client({ auth: process.env.NOTION_API_KEY });

    // Build IT page ID → slug map (for resolving FA slugs)
    console.log('[permitsFa.js] Building IT slug map...');
    const itSlugMap = await buildItSlugMap(notion);
    console.log(`[permitsFa.js] IT slug map has ${Object.keys(itSlugMap).length} entries`);

    // Fetch FA pages
    const faPages = await fetchDatabasePages(notion, FA_DATABASE_ID);
    console.log(`[permitsFa.js] Found ${faPages.length} FA permit pages`);

    if (faPages.length === 0) {
      console.warn('[permitsFa.js] No FA permit data found');
      return [];
    }
    ...
    for (const page of faPages) {
      ...
      if (!tipo) {
        console.warn(`[permitsFa.js] Skipping page ${page.id} - no Name`);
        continue;
      }
      ...
      if (!slug) {
        console.warn(`[permitsFa.js] Skipping "${tipo}" - no IT slug found (IT Page ID: ${itPageId || 'missing'})`);
        continue;
      }

      count++;
      if (count % 10 === 0) {
        console.log(`[permitsFa.js] Processed ${count}/${faPages.length} permits...`);
      }

      // Extract doc fields from FA page properties
      ...
      try {
        ...
        console.error(`[permitsFa.js] Error processing ${tipo}: ${err.message}`);
        processedPermits.push({
          id: page.id,
          slug,
          tipo,
    ...
    console.log(`[permitsFa.js] Cache: ${cacheHits} hits, ${cacheMisses} misses`);
    console.log(`[permitsFa.js] Returning ${processedPermits.length} FA permits`);
    return processedPermits;

  } catch (error) {
    console.error(`[permitsFa.js] Notion fetch failed: ${error.message}`);
    return [];
  }
};
