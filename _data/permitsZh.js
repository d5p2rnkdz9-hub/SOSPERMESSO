/**
 * 11ty data file for ZH permit pages
 * Fetches permit data from ZH Notion database, parses Q&A content into HTML
 * Slugs are resolved via IT Page ID → IT slug mapping (ZH slugs = IT slugs)
 */
...
// ZH Notion database ID (hardcoded like IT — no env var needed)
const ZH_DATABASE_ID = "e78a1d6a-a450-48ec-98dc-459f8a90ca32";
 ...
 * ZH version: no linkToDizionario, just escapeHtml for all text
 */
  ...
  if (block.type === 'paragraph') {
    ...
    if (richText && richText.length > 0) {
      ...
      if (firstSegment.annotations?.bold) {
        ...
        if (text.endsWith('?') || text.endsWith('？')) return { question: text };
      }
    }
  }
...
/**
 * Get emoji based on permit type keywords (ZH + EN + IT)
 */
function getEmojiForPermit(tipo) {
  ...
  // ZH + EN + IT keywords
  if (t.includes('study') || t.includes('studio') || t.includes('学习') || t.includes('学')) return '📖';
  if (t.includes('employed work') || t.includes('self-employment') || t.includes('lavoro subordinato') || t.includes('lavoro autonomo') || t.includes('工作')) return '💼';
  if (t.includes('job seeking') || t.includes('attesa occupazione') || t.includes('attesa lavoro') || t.includes('等待')) return '🔍';
  if (t.includes('protection') || t.includes('asylum') || t.includes('refugee') || t.includes('protezione') || t.includes('asilo') || t.includes('rifugiato') || t.includes('保护') || t.includes('庇护') || t.includes('难民')) return '🛡️';
  if (t.includes('family') || t.includes('famiglia') || t.includes('familiare') || t.includes('ricongiungimento') || t.includes('家庭') || t.includes('家人') || t.includes('团聚')) return '👨‍👩‍👧‍👦';
  if (t.includes('medical') || t.includes('health') || t.includes('pregnancy') || t.includes('medic') || t.includes('salute') || t.includes('gravidanza') || t.includes('cure') || t.includes('医疗') || t.includes('健康') || t.includes('怀孕')) return '🏥';
  if (t.includes('long-term') || t.includes('soggiornanti') || t.includes('lungo') || t.includes('长期居留')) return '🏠';
  if (t.includes('religion') || t.includes('motivi religiosi') || t.includes('宗教')) return '🕊️';
  if (t.includes('sport') || t.includes('attività sportiva') || t.includes('体育')) return '⚽';
  if (t.includes('art') || t.includes('artistico') || t.includes('艺术')) return '🎨';
  if (t.includes('research') || t.includes('ricerca') || t.includes('研究') || t.includes('科研')) return '🔬';
  if (t.includes('minor') || t.includes('minore') || t.includes('未成年')) return '👶';
  if (t.includes('disaster') || t.includes('calamit') || t.includes('灾害')) return '🌊';

  return '📄';
}

/**
 * Fetch and transform ZH permit data from Notion
 */
  ...
  if (!process.env.NOTION_FETCH) {
    try {
      const cached = JSON.parse(require('fs').readFileSync(
        require('path').join(__dirname, '..', '_cache', 'permits-zh.json'), 'utf-8'
      ));
      console.log(`[permitsZh.js] Using cached data (${cached.length} permits)`);
      return notionLinks.fixLinksInPermits(cached.map(p => ({ ...p, rinnovoDocuments: (p.rinnovoDocuments || []).filter(d => d.toLowerCase() !== 'n/a') })), 'zh/');
    } catch { /* no cache, fall through to Notion fetch */ }
  }

  if (!process.env.NOTION_API_KEY) {
    console.warn('[permitsZh.js] NOTION_API_KEY not set - returning empty array');
    return [];
  }

  try {
    console.log('[permitsZh.js] Fetching ZH permit data from Notion...');
    const notion = new Client({ auth: process.env.NOTION_API_KEY });

    // Build IT page ID → slug map (for resolving ZH slugs)
    console.log('[permitsZh.js] Building IT slug map...');
    const itSlugMap = await buildItSlugMap(notion);
    console.log(`[permitsZh.js] IT slug map has ${Object.keys(itSlugMap).length} entries`);

    // Fetch ZH pages
    const zhPages = await fetchDatabasePages(notion, ZH_DATABASE_ID);
    console.log(`[permitsZh.js] Found ${zhPages.length} ZH permit pages`);

    if (zhPages.length === 0) {
      console.warn('[permitsZh.js] No ZH permit data found');
      return [];
    }
    ...
    for (const page of zhPages) {
      ...
      if (!tipo) {
        console.warn(`[permitsZh.js] Skipping page ${page.id} - no Name`);
        continue;
      }
      ...
      if (!slug) {
        console.warn(`[permitsZh.js] Skipping "${tipo}" - no IT slug found (IT Page ID: ${itPageId || 'missing'})`);
        continue;
      }

      count++;
      if (count % 10 === 0) {
        console.log(`[permitsZh.js] Processed ${count}/${zhPages.length} permits...`);
      }

      // Extract doc fields from ZH page properties
      ...
      try {
        ...
        console.error(`[permitsZh.js] Error processing ${tipo}: ${err.message}`);
        processedPermits.push({
          id: page.id,
          slug,
          tipo,
    ...
    console.log(`[permitsZh.js] Cache: ${cacheHits} hits, ${cacheMisses} misses`);
    console.log(`[permitsZh.js] Returning ${processedPermits.length} ZH permits`);
    return processedPermits;

  } catch (error) {
    console.error(`[permitsZh.js] Notion fetch failed: ${error.message}`);
    return [];
  }
};
