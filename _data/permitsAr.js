/**
 * 11ty data file for AR permit pages
 * Fetches permit data from AR Notion database, parses Q&A content into HTML
 * Slugs are resolved via IT Page ID → IT slug mapping (AR slugs = IT slugs)
 */
...
// AR Notion database ID (hardcoded like IT — no env var needed)
const AR_DATABASE_ID = '39c16853-a650-4ab6-8cba-36e08a69fab8';
 ...
 * AR version: no linkToDizionario, just escapeHtml for all text
 */
...
/**
 * Get emoji based on permit type keywords (AR + IT)
 */
function getEmojiForPermit(tipo) {
  ...
  // AR keywords
  if (t.includes('دراس') || t.includes('تعليم') || t.includes('studio')) return '📚';
  if (t.includes('عمل') || t.includes('وظيف') || t.includes('lavoro subordinato') || t.includes('lavoro autonomo')) return '💼';
  if (t.includes('انتظار') || t.includes('بحث عن عمل') || t.includes('attesa occupazione') || t.includes('attesa lavoro')) return '🔍';
  if (t.includes('حماية') || t.includes('لجوء') || t.includes('لاجئ') || t.includes('protezione') || t.includes('rifugiato') || t.includes('asilo')) return '🛡️';
  if (t.includes('عائل') || t.includes('أسر') || t.includes('لم شمل') || t.includes('famiglia') || t.includes('familiare') || t.includes('ricongiungimento')) return '👨‍👩‍👧‍👦';
  if (t.includes('صح') || t.includes('علاج') || t.includes('حمل') || t.includes('medic') || t.includes('salute') || t.includes('gravidanza') || t.includes('cure')) return '🏥';
  if (t.includes('إقامة طويلة') || t.includes('soggiornante') || t.includes('lungo')) return '🏠';
  if (t.includes('دين')) return '⛪';
  if (t.includes('رياض')) return '⚽';
  if (t.includes('فن')) return '🎭';
  if (t.includes('بحث علمي') || t.includes('بحوث')) return '🔬';
  if (t.includes('قاصر') || t.includes('minore')) return '👶';
  if (t.includes('كارثة') || t.includes('calamit')) return '🌊';

  return '📋';
}

/**
 * Fetch and transform AR permit data from Notion
 */
module.exports = async function() {
  if (!process.env.NOTION_FETCH) {
    try {
      const cached = JSON.parse(require('fs').readFileSync(
        require('path').join(__dirname, '..', '_cache', 'permits-ar.json'), 'utf-8'
      ));
      console.log(`[permitsAr.js] Using cached data (${cached.length} permits)`);
      return notionLinks.fixLinksInPermits(cached.map(p => ({ ...p, rinnovoDocuments: (p.rinnovoDocuments || []).filter(d => d.toLowerCase() !== 'n/a') })), 'ar/');
    } catch { /* no cache, fall through to Notion fetch */ }
  }

  if (!process.env.NOTION_API_KEY) {
    console.warn('[permitsAr.js] NOTION_API_KEY not set - returning empty array');
    return [];
  }

  if (!AR_DATABASE_ID) {
    console.warn('[permitsAr.js] AR_DATABASE_ID not set - returning empty array');
    return [];
  }

  try {
    console.log('[permitsAr.js] Fetching AR permit data from Notion...');
    const notion = new Client({ auth: process.env.NOTION_API_KEY });

    // Build IT page ID → slug map (for resolving AR slugs)
    console.log('[permitsAr.js] Building IT slug map...');
    const itSlugMap = await buildItSlugMap(notion);
    console.log(`[permitsAr.js] IT slug map has ${Object.keys(itSlugMap).length} entries`);

    // Fetch AR pages
    const arPages = await fetchDatabasePages(notion, AR_DATABASE_ID);
    console.log(`[permitsAr.js] Found ${arPages.length} AR permit pages`);

    if (arPages.length === 0) {
      console.warn('[permitsAr.js] No AR permit data found');
      return [];
    }
    ...
    for (const page of arPages) {
      ...
      if (!tipo) {
        console.warn(`[permitsAr.js] Skipping page ${page.id} - no Name`);
        continue;
      }
      ...
      if (!slug) {
        console.warn(`[permitsAr.js] Skipping "${tipo}" - no IT slug found (IT Page ID: ${itPageId || 'missing'})`);
        continue;
      }

      count++;
      if (count % 10 === 0) {
        console.log(`[permitsAr.js] Processed ${count}/${arPages.length} permits...`);
      }

      // Extract doc fields from AR page properties
      ...
      try {
        ...
        console.error(`[permitsAr.js] Error processing ${tipo}: ${err.message}`);
        processedPermits.push({
          id: page.id,
          slug,
          tipo,
    ...
    console.log(`[permitsAr.js] Cache: ${cacheHits} hits, ${cacheMisses} misses`);
    console.log(`[permitsAr.js] Returning ${processedPermits.length} AR permits`);
    return processedPermits;

  } catch (error) {
    console.error(`[permitsAr.js] Notion fetch failed: ${error.message}`);
    return [];
  }
};
