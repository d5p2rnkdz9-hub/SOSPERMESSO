/**
 * 11ty data file for BN permit pages
 * Fetches permit data from BN Notion database, parses Q&A content into HTML
 * Slugs are resolved via IT Page ID → IT slug mapping (BN slugs = IT slugs)
 */
 ...
 * BN version: no linkToDizionario, just escapeHtml for all text
 */
...
/**
 * Get emoji based on permit type keywords (BN + IT)
 */
function getEmojiForPermit(tipo) {
  ...
  // BN keywords
  if (t.includes('শিক্ষা') || t.includes('পড়াশোনা') || t.includes('প্রশিক্ষণ') || t.includes('studio')) return '📚';
  if (t.includes('কাজ') || t.includes('চাকরি') || t.includes('নিয়োগ') || t.includes('lavoro subordinato') || t.includes('lavoro autonomo')) return '💼';
  if (t.includes('খোঁজা') || t.includes('অনুসন্ধান') || t.includes('attesa occupazione') || t.includes('attesa lavoro')) return '🔍';
  if (t.includes('সুরক্ষা') || t.includes('আশ্রয়') || t.includes('শরণার্থী') || t.includes('protezione') || t.includes('rifugiato') || t.includes('asilo')) return '🛡️';
  if (t.includes('পরিবার') || t.includes('পুনর্মিলন') || t.includes('famiglia') || t.includes('familiare') || t.includes('ricongiungimento')) return '👨‍👩‍👧‍👦';
  if (t.includes('স্বাস্থ্য') || t.includes('চিকিৎসা') || t.includes('গর্ভাবস্থা') || t.includes('medic') || t.includes('salute') || t.includes('gravidanza') || t.includes('cure')) return '🏥';
  if (t.includes('দীর্ঘমেয়াদী') || t.includes('soggiornante') || t.includes('lungo')) return '🏠';
  if (t.includes('ধর্ম')) return '⛪';
  if (t.includes('খেলাধুলা')) return '⚽';
  if (t.includes('শিল্প')) return '🎭';
  if (t.includes('গবেষণা') || t.includes('বিজ্ঞান')) return '🔬';
...
/**
 * Fetch and transform BN permit data from Notion
 */
module.exports = async function() {
  if (!process.env.NOTION_API_KEY) {
    console.warn('[permitsBn.js] NOTION_API_KEY not set - returning empty array');
    return [];
  }

  if (!BN_DATABASE_ID) {
    console.warn('[permitsBn.js] BN_DATABASE_ID not set - returning empty array');
    return [];
  }

  try {
    console.log('[permitsBn.js] Fetching BN permit data from Notion...');
    const notion = new Client({ auth: process.env.NOTION_API_KEY });

    // Build IT page ID → slug map (for resolving BN slugs)
    console.log('[permitsBn.js] Building IT slug map...');
    const itSlugMap = await buildItSlugMap(notion);
    console.log(`[permitsBn.js] IT slug map has ${Object.keys(itSlugMap).length} entries`);

    // Fetch BN pages
    const bnPages = await fetchDatabasePages(notion, BN_DATABASE_ID);
    console.log(`[permitsBn.js] Found ${bnPages.length} BN permit pages`);

    if (bnPages.length === 0) {
      console.warn('[permitsBn.js] No BN permit data found');
      return [];
    }
    ...
    for (const page of bnPages) {
      const tipo = page.properties["Name"]?.title?.[0]?.plain_text || null;
      if (!tipo) {
        console.warn(`[permitsBn.js] Skipping page ${page.id} - no Name`);
        continue;
      }
      ...
      if (!slug) {
        console.warn(`[permitsBn.js] Skipping "${tipo}" - no IT slug found (IT Page ID: ${itPageId || 'missing'})`);
        continue;
      }

      count++;
      if (count % 10 === 0) {
        console.log(`[permitsBn.js] Processed ${count}/${bnPages.length} permits...`);
      }

      // Extract doc fields from BN page properties
      ...
      try {
        ...
        console.error(`[permitsBn.js] Error processing ${tipo}: ${err.message}`);
        processedPermits.push({
          id: page.id,
          slug,
          tipo,
    ...
    console.log(`[permitsBn.js] Cache: ${cacheHits} hits, ${cacheMisses} misses`);
    console.log(`[permitsBn.js] Returning ${processedPermits.length} BN permits`);
    return processedPermits;

  } catch (error) {
    console.error(`[permitsBn.js] Notion fetch failed: ${error.message}`);
    return [];
  }
};
