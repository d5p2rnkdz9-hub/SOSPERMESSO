...
│   ├── updates.js               # "Ultimi aggiornamenti" banner (11 languages)
...
## Normativa — testi coordinati interattivi

`/normativa.html` (IT-only, `src/pages/normativa.html` + `_data/normativa.js`) presents the
Italian norms coordinated with the **d.l. 12 giugno 2026, n. 100** (the decree aligning Italian
law with the EU Pact): T.U. immigrazione (D.Lgs. 286/98), D.Lgs. 25/2008, 142/2015, 251/2007.

The readers themselves are a **static bundle generated by the sospatto.it pipeline**, committed
under `public/patto-interattivo/` (~37 MB: 10 EU regulations, 4 Italian texts, a hub page, and
external-decision stubs). Each reader is a self-contained app (TOC, clickable cross-references,
`ins`/`del` highlighting of the d.l. 100/2026 changes) with **relative links between texts**, so
the bundle must stay in one directory.

- `eleventy.config.mjs` maps it to `/patto-interattivo/...` (not `/public/...`); the rest of
...
Header navigation (IT — see `_data/nav.js`, translated navs are shorter):
- **Info**: permessi, documenti Questura, circolari, guide, aiuto legale, dizionario
- **Patto UE** (badge NUOVO): external link to sospatto.it
- **Normativa**: the four Italian coordinated texts (`/normativa.html#<slug>`), IT-only
- **Test interattivi**: app.sospermesso.it decision trees
- **Contattaci**: app.sospermesso.it forms

Only one badge lives in the visible bar at a time (Patto UE) — a second one made the
labels wrap. New items carry their badge inside the dropdown instead. The desktop bar is
tuned in three steps below 1200px (`components.css`, "Desktop stretto"): five dropdowns
...
- `updates-banner.liquid` — "Ultimi aggiornamenti" scrolling ticker right below the menu,
  on every page of all 11 languages. Content lives in `_data/updates.js` (one entry per
  language, same shape as `nav.js`; newest item first). All items scroll in one continuous
  marquee at a fixed ~60 px/s: the markup renders two copies of the item group, and the
  inline script measures a group, clones more if the viewport is wider, and sets
  `--updates-shift` / `--updates-duration`. Scrolling pauses on hover, on keyboard focus and
  via the pause button (WCAG 2.2.2); `prefers-reduced-motion` turns the ticker into plain
  wrapping text and drops the button. Styles live in the "ULTIMI AGGIORNAMENTI" section of
  `components.css` — note the `max-width: none` there, needed against `* { max-width: 100% }`
  in `mobile-fix.css`.
