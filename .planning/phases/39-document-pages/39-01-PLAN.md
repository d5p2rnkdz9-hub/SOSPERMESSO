...
must_haves:
  truths:
    ...
    - "Helper functions (linkToDizionario, normalizeDocumentName, etc.) are available as Liquid filters"
  ...
  artifacts:
    - path: "_data/documents.js"
      ...
      exports: ["documents array with primo/rinnovo data"]
...
Output: `_data/documents.js` that exports document array, Liquid filters registered in eleventy.config.mjs
</objective>
...
<task type="auto">
  ...
  <action>
Create `_data/documents.js` that exports an async function returning array of document objects.
...
Return array of document objects. Each object represents ONE page type (primo or rinnovo), so expand each permit into separate primo and rinnovo entries for easier pagination later:

```javascript
// Example output structure:
[
  { tipo: "Studio", slug: "studio", type: "primo", documents: [...], method: "KIT", docNotes: "..." },
  { tipo: "Studio", slug: "studio", type: "rinnovo", documents: [...], method: "KIT", docNotes: "..." },
  ...
]
```

**Graceful degradation:** If NOTION_API_KEY is not set or API fails, return empty array and log warning (don't fail build).

Use CommonJS format (not ESM) per established pattern.
  </action>
  <verify>
Run `node -e "require('./_data/documents.js')().then(d => console.log('Found:', d.length, 'pages'))"` - should output page count or "Found: 0 pages" if no API key.
  </verify>
  <done>
_data/documents.js exists, exports async function, returns document array (or empty array gracefully).
  </done>
</task>
...
Required filters (adapt from scripts/templates/helpers.js):
...
7. **formatNotesContent** - Format notes content as HTML
   - Handle bullet points, intro text, after text
   - Return HTML string
...
Import helpers.js functions or inline the logic (helpers.js is CommonJS, config is ESM - may need to inline or use createRequire).
  </action>
  <verify>
Run `npx @11ty/eleventy --dryrun` - should complete without errors about unknown filters.
  </verify>
  <done>
eleventy.config.mjs has 7 new filters registered: linkToDizionario, normalizeDocumentName, getDocumentClass, isDisputed, escapeHtml, parseDocNotes, formatNotesContent.
  </done>
</task>
...
<verification>
1. `node -e "require('./_data/documents.js')().then(d => console.log(d.length))"` - returns number or 0
...
<success_criteria>
1. _data/documents.js fetches from Notion and returns document array
2. _data/slugMap.js provides redirect mappings
3. eleventy.config.mjs has all 7 helper filters registered
