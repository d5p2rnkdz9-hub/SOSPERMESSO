...
must_haves:
  ...
  key_links:
    - from: "src/pages/documents-primo.liquid"
      ...
      via: "pagination over documents.primo"
      pattern: "pagination.*documents\\.primo"
    - from: "src/pages/documents-rinnovo.liquid"
      to: "_data/documents.js"
      via: "pagination over documents.rinnovo"
      pattern: "pagination.*documents\\.rinnovo"
    - from: "templates"
      to: "filters"
...
Purpose: Replace JS template functions (primo.js, rinnovo.js) with Liquid templates. Each page is generated from the documents data arrays using size:1 pagination.
...
**Prerequisite:** Plan 39-01 created `_data/documents.js` returning `{primo: [...], rinnovo: [...]}` structure.
...
3. **Prepend common document** - render "Copia permesso precedente" first, then loop through remaining docs:
...
<div class="doc-checklist" data-permit="{{ doc.slug }}-rinnovo">
  ...
  <div class="{{ commonDocClass }}">
    ...
    <span class="doc-label">{{ commonDocLabel | safe }}</span>
  </div>
...
4. Add renewal alert after the standard warning:
```liquid
...
5. Add attesa-occupazione warning (before checklist section):
```liquid
...
<section class="section" style="padding: 0.5rem 0;">
  <div class="container" style="max-width: 700px;">
    <div class="alert alert-danger" style="background: #FFF0F0; border: 1.5px solid #E53935; border-radius: 8px;">
      <span class="alert-icon">⚠️</span>
      <div>
        <strong>Attenzione:</strong> Il rinnovo del permesso per attesa occupazione è controverso...
...
6. Update progress count to account for the prepended common document:
```liquid
