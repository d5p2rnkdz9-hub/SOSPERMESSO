...
must_haves:
  truths:
    - "Every page in src/pages/ displays the yellow footer with 'Il Progetto' link"
  ...
  artifacts:
    - path: "src/pages/*.html"
      provides: "Yellow footer on all 138 pages"
...
Purpose: Achieve visual consistency across all 138 pages - every page should display the new yellow footer with "Il Progetto" and "Contatti" links, matching the template established in Phase 12.

Output: All pages in src/pages/ updated with new footer structure
</objective>
...
# Count files with new footer (should be 138)
...
- 138 files have new footer-project-link pattern
...
```
  ...
  <verify>
All 138 files use `href="chi-siamo.html"` (same-directory relative path) for Il Progetto link.
All 138 files use `href="https://form.typeform.com/to/USx16QN3"` for Contatti link.
  </verify>
  <done>
...
<success_criteria>
1. All 138 pages in src/pages/ have the yellow footer with footer-project-link class
