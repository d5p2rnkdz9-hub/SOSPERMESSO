...
2. Verify export includes both functions:
   `node -e "const t = require('./scripts/templates/permesso.js'); console.log(Object.keys(t))"`
   Should include: generatePermessoPage, generatePlaceholderPage
  </verify>
...
<task type="auto">
  ...
  <files>scripts/build-permits.js</files>
  <action>
**FIRST:** Verify the import from Task 1 works before using the function.

Run this test at the START of implementing this task:
```bash
node -e "const t = require('./scripts/templates/permesso.js'); if (typeof t.generatePlaceholderPage !== 'function') { console.error('ERROR: generatePlaceholderPage not found'); process.exit(1); } console.log('OK: generatePlaceholderPage available');"
```

If this test fails, Task 1 is incomplete - do not proceed.

**THEN:** Update build-permits.js to generate placeholder pages instead of skipping empty permits.

Modify the build() function:

1. Import generatePlaceholderPage at the top of the file (add to existing require):
   ```javascript
