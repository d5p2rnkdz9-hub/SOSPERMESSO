...
must_haves:
  ...
  key_links:
    - from: "src/pages/index.html"
      to: "database.html"
      via: "href in Database section card (nested context - NO src/pages/ prefix)"
      pattern: 'href="database.html"'
      note: "Nested pages link to siblings via simple filename, not full path"
    - from: "src/pages/index.html"
      to: "aiuto-legale.html"
      via: "href in Aiuto legale section (nested context - NO src/pages/ prefix)"
      pattern: 'href="aiuto-legale.html"'
    - from: "src/pages/chi-siamo.html"
      to: "../../index.html"
      via: "nav menu Home link (goes up 2 dirs to root)"
      pattern: 'href="../../index.html"'
    - from: "src/pages/index.html"
      to: "contact modal"
      via: "openContactModal() onclick and fetch('../components/contact-form.html')"
      pattern: "fetch\\('\\.\\./components/contact-form\\.html'\\)"
      note: "Nested context uses ../ to go up one dir to src/, not src/ prefix"
---
...
8. Run: `grep 'grid grid-2' src/pages/index.html` - should return at least 1 match (Database section uses grid-2)
  </verify>
  <done>
...
1. **ADD Aiuto legale section** (NEW - insert after Guides, before Link utili):
   ...
   <section class="section aiuto-legale-section" style="background: linear-gradient(135deg, #FFEBEE 0%, #FFCDD2 100%);">
     <div class="container">
       <div class="section-header">
         <h2 class="section-title" style="color: var(--lighthouse-red);">AIUTO LEGALE ⚖️</h2>
       ...
       <a href="aiuto-legale.html" class="card card-link aiuto-legale-card">
         ...
         <button class="btn btn-secondary">TROVA AIUTO →</button>
       </a>
     </div>
   </section>
   ```
...
4. **Update nav menu** (line 43-48):
   ...
   - Change "Guide" link to point to root homepage: `href="../../index.html#guide"`
   - Change "Test" link to point to root homepage: `href="../../index.html#test"`
   - Match root index.html nav structure (root uses #guide, #test; nested must use full path to root)
...
Path notes (CRITICAL - nested context):
- Contact form fetch: `fetch('../components/contact-form.html')` (note: ../ goes up to src/, NOT src/ prefix)
- Nav links to root sections: `../../index.html#guide` and `../../index.html#test` (goes up 2 dirs to root)
  </action>
  <verify>
...
7. Verify script loads contact form via `fetch('../components/contact-form.html')` (NOT `src/components/`)
8. Run: `grep "fetch('../components/contact-form.html')" src/pages/index.html` - should return 1 match
9. Click "Scrivici" button - verify modal opens (not external typeform link)
10. Run: `grep "AIUTO LEGALE" src/pages/index.html` - should return 1 match
11. Run: `grep "openContactModal" src/pages/index.html` - should return at least 1 match
12. Run: `grep "contact-form.html" src/pages/index.html` - should return 1 match
  </verify>
  <done>
