  ...
  - service: notion
    ...
    dashboard_config:
      ...
      - task: "Share database with integration"
        location: "Open database → ... menu → Connections → Add [your integration name]"

must_haves:
  truths:
    - "npm install runs without error"
    - "Notion client can authenticate with API key"
...
Note on DATABASE_ID: Remove hyphens from the UUID (1ad7355e-7f7f-80bc-b445-000b881c6c80 → 1ad7355e7f7f80bcb445000b881c6c80) as Notion API requires 32-character hex string without dashes.
  </action>
  <verify>
...
async function build() {
  ...
  if (!connected) {
    console.log('⚠️  Skipping build - Notion not configured');
