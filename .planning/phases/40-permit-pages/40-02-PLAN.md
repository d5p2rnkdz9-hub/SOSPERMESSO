...
@scripts/templates/permesso.js — HTML structure to replicate in Liquid. Read ALL four generate functions: generatePermessoPage (lines 82-292), generatePlaceholderPage (lines 302-492), generateVariantParentPage (lines 504-745), generateVariantChildPage (lines 760-978). These contain the exact HTML structure, class names, and section ordering that the Liquid template must reproduce.
    ...
    <div class="breadcrumb-content">
      <div class="breadcrumb-nav">
        ...
        {% if permit.isVariantChild %}
          <a href="permesso-{{ permit.parentSlug }}.html">Permesso per {{ permit.baseName }}</a> →
          <span>{{ permit.variantName }}</span>
        {% else %}
          <span>Permesso per {{ permit.tipo }}</span>
        {% endif %}
      </div>
...
IMPORTANT: Use literal `→` character for breadcrumb arrows, NOT `&rarr;` entity. This matches the existing static pages (see permesso-studio.html line 16-17 which uses literal `→`).

IMPORTANT: Use `permit.parentSlug` for the variant child breadcrumb link — NOT `permit.baseName | slugify`. Liquid does not have a built-in slugify filter, and parentSlug is pre-computed in the data file (Plan 40-01).
...
<section class="section bg-off-white">
  <div class="container">
    <div class="page-header text-center">
      ...
      <h1 class="page-title">Permesso per {{ permit.tipo }}</h1>
    </div>
  </div>
</section>
```
...
Read `generateVariantParentPage` in `scripts/templates/permesso.js` lines 504-745 and replicate its HTML structure in Liquid. The key sections are:

- Info alert box: `<div class="alert alert-info">` explaining "informazioni comuni a tutte le tipologie" (lines 671-680 of permesso.js)
- Default generic cards when no general sections exist — three cards for "Cos'e il permesso per [baseName]?", "Durata", and "Che diritti mi da?" with generic placeholder content (lines 512-531 of permesso.js)
- "Quale situazione ti interessa?" heading with `<div class="grid grid-2">` of variant link cards (lines 687-694)
- Each variant card: `<a href="permesso-{{ variant.slug }}.html" class="card card-link">` with variant emoji and name. Include variant.variantName in the card.

**B) Placeholder page (permit.isPlaceholder == true AND NOT isVariantParent):**
Read `generatePlaceholderPage` in `scripts/templates/permesso.js` lines 302-492 and replicate:

- "Contenuto in arrivo" card: `<div class="card text-center" style="padding: 3rem;">` with emoji, heading, message text, and "Dai una mano" CTA button linking to Typeform (lines 426-439 of permesso.js)

**C) Variant Child (permit.isVariantChild == true AND NOT isPlaceholder):**
Read `generateVariantChildPage` in `scripts/templates/permesso.js` lines 760-978 and replicate:

- Back-to-parent link: `<a href="permesso-{{ permit.parentSlug }}.html">` with left arrow and "Torna alle informazioni generali su [baseName]" (lines 891-897 of permesso.js). Use `permit.parentSlug` NOT `permit.baseName | slugify`.
- Info alert explaining "solo le informazioni specifiche per [variantName]" (lines 902-911)
- Q&A sections loop (same as standard)
- "Hai altre domande?" CTA

**D) Standard permit (default — has sections, not variant parent):**
Read `generatePermessoPage` in `scripts/templates/permesso.js` lines 82-292 and replicate:

- Document CTA buttons (primo rilascio + rinnovo) — lines 211-222
- Q&A sections loop:
  ```liquid
  ...
  {% endfor %}
  ```
- "Hai altre domande?" CTA — lines 229-238

**4. RELATED section** (all page types):
- Three cards: Tutti i permessi, Documenti Questura, Dizionario (matching current pages, lines 244-262)
...
NOTE on placeholder pages: Although placeholder handling is not explicitly listed in the ROADMAP PERM requirements, it is included here to match existing behavior. The current build-permits.js generates placeholder pages for permits without Notion content, and removing this would break existing URLs. This is an implicit requirement for URL preservation (PERM-04).
...
`ls _site/src/pages/permesso-studio.html 2>/dev/null` (or whichever permit exists)
  ...
  <done>
permits.liquid template exists with pagination, conditional rendering for all 4 page types (standard, placeholder, variant parent, variant child), uses base layout, uses permit.parentSlug for variant child breadcrumbs, uses literal arrow character
  </done>
</task>
...
7. Variant child pages have back-to-parent link using parentSlug and Q&A sections
</verification>
