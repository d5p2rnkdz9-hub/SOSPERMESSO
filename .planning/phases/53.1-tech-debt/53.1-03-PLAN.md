...
**Preferred approach:** Use Liquid variables directly. The base.liquid already computes `itUrl`, `enUrl`, `frUrl`. However, these are computed inside base.liquid's head section. Instead, replicate the URL computation in nav.liquid (or better yet, compute once in base.liquid and store in variables that are accessible to includes).

Actually, the simplest approach: Compute language URLs in nav.liquid using the same Liquid logic as base.liquid:
...
The existing nav dropdown CSS (`.has-dropdown` hover behavior) and mobile nav toggle behavior will handle the language dropdown automatically since it uses the same HTML structure.
  ...
  <done>Language switcher is a dropdown item inside the nav menu, not a standalone component in the header. All 3 languages are listed with links to equivalent pages.</done>
</task>
