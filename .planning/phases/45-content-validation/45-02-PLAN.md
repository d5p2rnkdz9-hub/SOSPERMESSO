...
must_haves:
  truths:
    - "Phase 43 rule violations are fixed directly in Notion permit pages"
    - "Document DB fields (Info extra su doc rilascio/rinnovo) are fixed for Phase 43 rule violations"
...
**Priority 4: Fix document page issues from validation findings**

Process the `documents.pages` array from validation-findings.json. Document page content comes from Notion DB columns (not page blocks), so fixes go to the database properties:

- **note_rule_violations** (BOLLETTINO_INCLUDES_40, FULL_URLS, TU_TONE in `Info extra su doc rilascio` or `Info extra su doc rinnovo` fields): Fix the DB column value using `notion.pages.update()` with corrected rich_text. These columns are the source for document page notes sections.
- **DOCUMENT_LIST_MISMATCH**: Log mismatches in the report for user review (could be a normalization issue in the template or a genuine missing doc in the DB).
- **EMPTY_DOCUMENT_PAGE**: Log in report (permits with no documents defined).
- **MISSING_NOTES**: Log in report if DB has `Info extra` but HTML doesn't render it (likely a template issue).

For each document DB field fix, use:
```
notion.pages.update({
  page_id: permitId,
  properties: {
    "Info extra su doc rilascio": {
...
Track all document fixes in the same changes log for the report.
  </action>
  <verify>
...
<verification>
1. All Phase 43 rule violations found in Plan 01 are fixed in Notion (permit pages AND document DB fields)
...
5. Document page issues logged or fixed as appropriate
6. VALIDATION-REPORT.md exists with accurate, complete information
</verification>

<success_criteria>
- Rule violations fixed directly in Notion (NO_DOC_LISTS, BOLLETTINO_INCLUDES_40, FULL_URLS, TU_TONE) for both permit pages and document DB fields
- Missing sections added from DB column data
- Conflicts logged for user review (never auto-resolved)
- Document page issues (mismatches, empty checklists) documented in report
