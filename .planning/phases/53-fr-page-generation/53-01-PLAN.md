...
<objective>
Fix the EN DuplicatePermalinkOutputError blocker and create all FR pagination templates so 11ty generates French permit and document pages from Notion data.

Purpose: This is the core generation pipeline -- without it, no FR pages exist. The blocker fix is prerequisite for any build to succeed.
Output: 5 new files (FR pages.11tydata.js + 3 FR liquid templates) + 3 modified files (eleventy.config.mjs + 2 EN document templates with FR hreflang)
</objective>
...
<task type="auto">
  <name>Task 2: Create FR pagination templates (permits + documents primo + documents rinnovo) + update EN document hreflang</name>
  <files>
    fr/src/pages/permits-fr.liquid
    fr/src/pages/documents-primo-fr.liquid
    fr/src/pages/documents-rinnovo-fr.liquid
...
5. Check that `data-permit` attributes use `-fr` suffix in FR document templates
  ...
  <done>
Three FR pagination templates exist and follow EN patterns with French UI strings. EN document templates updated with FR hreflang. All internal links point to /fr/ prefix. Checklist localStorage keys use -fr suffix.
  </done>
</task>
...
5. No `/en/` paths remain in FR templates (search for `/en/` in all FR files)
</verification>
