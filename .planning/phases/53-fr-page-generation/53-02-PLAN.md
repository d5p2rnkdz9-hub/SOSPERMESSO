...
Note: The existing template includes flag emojis in the span. Keep whatever format the existing template uses (check if it says "EN" or "EN [flag]"). Looking at the actual file, it uses `EN [gb_flag]` and `IT [it_flag]`. So use `FR [fr_flag]` for consistency. The exact existing format is `EN` and `IT`, so use `FR` for consistency.
...
6. Verify app.js is syntactically valid: `node --check src/scripts/app.js` (must exit 0 with no errors)
  </verify>
  <done>
Header logo links to /fr/ on FR pages. Language switcher shows "FR" on FR pages. app.js enables FR detection and three-way path switching (IT/EN/FR) and passes syntax check. base.liquid emits hreflang tags for all three languages with correct URL derivation.
  </done>
</task>
...
5. app.js passes `node --check` syntax validation
6. base.liquid hreflang includes `fr` alternate for all page types (IT, EN, FR)
7. No hardcoded `en` references in FR-specific paths
</verification>
